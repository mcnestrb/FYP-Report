...
matching supply and demand in the face of unpredictable supply. In the course of
this project, several papers were surveyed, before deciding on the paper
authored by Wayes Tushar \cite{tushar2014prioritizing} to form the basis of the
project and therefore this report. This paper was selected due to the importance
it placed on minimal communication and the privacy which is provides to each of
the actors within the system, that is that they do not share their private
information with everyone involved. The system will then be compared against a
simple scenario of current approaches to solving this problem to see whether or
not this particular smart grid solution would be effective in a real world
situation.

In order to determine the success of this project, it will try to prove whether
a game theoretic approach to the problem of matching supply and demand in the
face of unpredictable supply is feasible within a real network situation. In
order to do this, the game theoretic approach will first be implemented and then
analysed to see whether it is suitable and is in fact able to solve the problem
in an amount of time that would make it fit to use in the real world. That is,
that the system would match supply and demand in a realistic amount of time. The
report will also analyse the appropriateness of the implementation of a game
theoretic approach and the usefulness, or lack thereof, of mathematical
...
implementation of managing supply, definitions of auctions and game
...
finally the actual implementation itself will be presented. This will then be
followed by a small discussion on the implementation and running of the system.
To conclude there will be an assessment of the system designed and a number of
potential continuations of the project will be discussed.
\part{Background}
\label{sec:orga0ade47}
\chapter{Decentralised Grid}
\label{sec:org0d726c2}
...
\end{figure}
\chapter{Smart Grid}
\label{sec:org9d9ab52}
\section{Overview}
\label{sec:org28377c8}
...
\chapter{Auctions}
\label{sec:org35451fc}
\section{Overview}
\label{sec:orgd84c735}
...
\chapter{Game Theory}
\label{sec:orgd62b37e}
\section{Overview}
\label{sec:org428ff25}
...
\label{sec:orgb39d0a2}
\section{Overview}
\label{sec:org176483e}
...
\part{Implementation}
\label{sec:org407ad99}
\chapter{Design}
\label{sec:org5f99eea}
\section{Games vs Auctions}
\label{sec:org444fca0}
...
\label{sec:orga8bcee1}
\subsection{EC Utility Function}
\label{sec:org49e12f7}
...
\chapter{Application}
\label{sec:org0d88cbb}
\section{Python Twisted Framework}
\label{sec:org3d2942b}
...
\part{Conclusion}
\label{sec:org9221133}
\chapter{Further Work}
\label{sec:org0421b92}
...
While the proliferation of smart meters as certainly improved, an important
...
single consumer. In this instance some kind of intermediate device would be used
to collect the demand at the start of the system and to distribute the energy at
the end.
...
device compared to sending information across the network. For example, in this
project convex optimisation was used by the CPS to determine the price vector
based on the provided energy estimates. The new prices were calculated rapidly
and then sent out to each of the ESs within that particular game. The
...
One detractor from the scalability of the system was the game itself as it
turned out. In order to determine Nash Equilibrium within the game, the slack
